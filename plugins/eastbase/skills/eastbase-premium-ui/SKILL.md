...
description: >-
  ...
  (BurnCap's "Ember Console" is the reference). Read this BEFORE writing or editing ANY
  user-facing UI in an eastbase / BurnCap project — including small, routine-seeming
  tasks. Critical: eastbase UI uses specific, non-obvious house patterns you do NOT
  already know (instrument-cluster KPI grids, hand-built signature gauges, mono-typeset
  ranked tables, atmosphere without gradients, signature-branded empty / loading / error
  states); the stock-shadcn or generic-SaaS version you would write from memory is the
  wrong answer here, so consult the patterns first even for a single component or state.
  Covers: any page, dashboard, analytics or admin view, settings or onboarding screen;
  any chart, KPI row, stat cluster, or data table; ESPECIALLY empty states, loading
  skeletons, and error / 404 / 500 screens; and any request to review, polish, redesign,
  or de-genericize a component — even when the user never says "premium" or "design" or
  names this skill. Do NOT trigger for non-UI work (backend, DB migrations, business
  logic, data / metrics questions), purely functional bugfixes (e.g. a layout overflow),
  dependency or tooling setup (installing a chart library, adding a stock shadcn
  component), or UI for non-eastbase projects.
---
...
This is a **method**, not a paint job. BurnCap's "Ember Console" design language is the
fully-worked reference, available in `references/burncap-ember-console.md`. Do **not**
copy BurnCap's ember hue or flame metaphor into an unrelated product — copy the *moves*
and re-derive the specifics from the new product's own domain.
...
1. **Find the domain metaphor.** What does this product actually *do*, and what real
   ...
   **instrument console** with **gauges** and an **ember**. A scheduling product might
   be a *control board*; a writing tool a *manuscript/press*; a security product a
   *vault/perimeter*. The metaphor names the design language (BurnCap → "Ember
   Console") and decides what your components want to look like.
...
3. **Typeset the product.** Choose a small type system with distinct roles: a
   **display** face for ceremony (hero, auth, big empty-state headlines), a clean
   **UI** sans for everything functional, and — critically — a **monospace** for all
   numerals and data. Mono tabular figures are the cheapest, strongest signal that a
   surface is an *instrument* and not a brochure.
4. **Decide the atmosphere.** How does a flat dark (or light) page gain depth *without*
   gradient fills? Pick the product's texture kit: film grain, a hairline blueprint
   grid, a single blurred signature-color glow, hairline borders, soft panel shadows.
5. **Name a small component vocabulary.** The 5–8 bespoke primitives every screen
   ...
   chrome. See the BurnCap set in the reference.
...
- **One signature hue + a small semantic palette.** Color carries meaning; it is never
  ...
  it.
- **Atmosphere, not gradients.** Depth comes from grain, hairline grids, a blurred
  solid signature-glow, hairline borders, and soft shadows — **no linear-gradient
  backgrounds** unless the user explicitly asks. Gradients are the house tell of
  generic AI UI; avoid them.
- **Data is typeset.** Numerals are monospace and `tabular-nums`, right-aligned in
  tables, so figures line up and read like instrument readouts.
- **Instruments over widgets.** Prefer a purpose-built visualization that embodies the
  metaphor over a default chart/card (see §3).
...
- **Dark mode and contrast are first-class**, not a `dark:` afterthought. Many eastbase
  products are dark-primary. Whichever you choose, both themes must be deliberately
  tuned and meet contrast (see §6).
...
This is where most dashboards go generic. Charts, tables, analytics, and admin views
are the heart of eastbase products, so they get the most invention — not a dropped-in
chart library with default styling.

Principles (BurnCap code patterns for each are in
`references/burncap-ember-console.md` — adapt, don't paste):
...
- **KPI rows are an instrument cluster, not a row of cards.** Render tiles as a single
  ...
  a signature accent edge. Big mono numerals.
...
- **Tables are ranked and typeset, not spreadsheets.** Mono `01/02` rank numerals,
  the leader in the signature hue, right-aligned mono figures, and an **inline share
  bar** so each row shows magnitude at a glance. A dense table can be beautiful.
- **Show composition as one segmented bar**, not a pie — a single hairline-split bar
  with a mono legend gives "what's my mix" instantly.
- **When you do use a chart library** (recharts etc.), retheme it completely: chart
  colors from your tokens, mono tabular tooltips with a color swatch, axis lines off /
...
The user singled this out, and it's the fastest way to tell a crafted product from a
generic one. Empty, error, and loading are not edge cases — they're the **first** thing
a new user sees and the moments a stressed user remembers. Give each the product's
signature. Full patterns and code in `references/state-ui-signatures.md`.

- **Empty / first-run is a designed moment**, not centered gray text. A full-bleed
  dashed panel with the signature glow + grain, the brand mark in an accent chip, a
  **display-font** headline, one human sentence, and a clear primary action (plus a
  "load demo data" escape hatch where it fits). Inline empties (an empty panel/table)
  get one *forward-looking* sentence — "No alerts yet. Budgets and spike rules will
  post here." — not a dead "No data".
...
- **Loading mirrors the real layout.** Skeletons shaped like the actual page (cluster,
  panels, varied-height chart bars) so content pops in without jank — plus a small live
  branded spinner + label so it reads as an *instrument warming up*, not a frozen page.
  Reserve a centered branded loader for light pages (onboarding/invite).
...
- **Reserve ceremony type for ceremony.** Display/serif faces belong on marketing,
  auth, and big empty-state headlines — not on dense dashboard data.

## 6. Quality gates

Before calling any UI done, verify (use the preview tools — don't eyeball from code):

- **Responsive.** The instrument cluster reflows (e.g. 2-col → 5-col), tables stay
  legible or scroll deliberately, nothing overflows. Check a narrow viewport.
- **Dark + light.** Both themes are deliberately tuned — signature glow, borders, and
  status hues all read correctly in each. No invisible borders, no blown-out glows.
- **Contrast.** Body text, muted text on tinted panels, and status colors on their
  surfaces meet WCAG AA. Warm/desaturated palettes are easy to push too low — check.
...
2. **Signature present** — one hue + small semantic palette, mono typeset data, the texture kit (no stray gradients)?
3. **Instruments, not widgets** — is the hero data shape bespoke and metaphor-driven, or a default chart?
...
6. **Gates pass** — responsive, dark+light, contrast, reduced-motion all verified in the browser?

## References

- `references/burncap-ember-console.md` — the fully-worked reference implementation:
  tokens, the shared primitive vocabulary, the instrument cluster, gauges, ranked
  tables, segmented composition, and a retheme'd recharts chart. Read this for concrete
  code patterns to adapt (the "check BurnCap components" the brief asked for).
